...
import (
	...
	"github.com/mariana-reis/chat-service/internal/infra/db"
)
...
func (r *ChatRepositoryMySQL) SaveChat(ctx context.Context, chat *entity.Chat) error {
	...
	err = r.Queries.DeleteChatMessages(ctx, chat.ID) // delete messages
	if err != nil {
		return err
	}

	err = r.Queries.DeleteErasedChatMessages(ctx, chat.ID) // delete erased messages
	if err != nil {
		return err
	}
	// save messages
